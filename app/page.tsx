...
export default async function Home() {
  ...
  const popular = [...news].sort((a, b) => {
    const av = newsRecords.find((item) => item.slug === a.slug)?.viewCount ?? 0
    const bv = newsRecords.find((item) => item.slug === b.slug)?.viewCount ?? 0
    return bv - av
  }).slice(0, 4)
  const upcoming = events.slice(0, 3)
  const islamicHighlights = islamicItems.slice(0, 3)
  const announcements = announcementRecords.slice(0, 3).map((item) => ({
    date: formatIndonesianDate(item.publishedAt, false),
    title: item.title,
    href: '/pengumuman/',
  }))
    ...
    <SiteShell>
      <main id="main-content">
        ...
        <section className="home-news container" aria-labelledby="berita-terbaru">
          ...
          {featured ? <div className="home-news-layout">
          ...
          </div> : <div className="empty-state"><strong>Belum ada berita</strong><p>Belum tersedia berita yang dipublikasikan.</p></div>}
        </section>
          ...
          <div className="home-editorial-grid">
            <div>
              ...
              <div className="announcement-list">
                ...
                {announcements.length === 0 && <div className="empty-state compact"><strong>Belum ada pengumuman</strong><p>Belum tersedia informasi resmi.</p></div>}
              </div>
            </div>
            <div>
        ...
        <section className="home-agenda container" aria-labelledby="agenda-terdekat">
          ...
          {upcoming.length > 0 ? <div className="events-grid">{upcoming.map((event) => <Link className="event-card" key={event.slug} href={`/kegiatan/${event.slug}/`}><div className="event-date"><strong>{event.day}</strong><span>{event.month}</span></div><div><span className="event-label">{event.category}</span><h3>{event.title}</h3><p><CalendarDays size={15} /> {event.time}</p><p><MapPin size={15} /> {event.place}</p></div></Link>)}</div> : <div className="empty-state"><strong>Belum ada agenda</strong><p>Belum tersedia kegiatan mendatang.</p></div>}
        </section>
        ...
        <section className="home-islamic container" aria-labelledby="ruang-keislaman">
          ...
          {islamicHighlights.length > 0 ? <div className="islamic-home-grid">{islamicHighlights.map((item, index) => <Link className="islamic-home-card" key={item.slug} href={`/keislaman/${item.slug}/`}><span className="islamic-index">0{index + 1}</span><span className="eyebrow">{item.category}</span><h3>{item.title}</h3><p>{item.excerpt}</p><strong>Pelajari <ArrowRight size={15} /></strong></Link>)}</div> : <div className="empty-state"><strong>Belum ada materi</strong><p>Belum tersedia materi keislaman.</p></div>}
        </section>
