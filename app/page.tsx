...
import { ArrowRight, CalendarDays, ChevronRight, MapPin, Sparkles } from 'lucide-react'
import SiteShell from '@/components/layout/SiteShell'
import LiveNews from '@/components/live/LiveNews'
import { contentRepository } from '@/lib/data'
import { eventRecordToLegacy, formatIndonesianDate, islamicRecordToLegacy, newsRecordToLegacy } from '@/lib/data/presentation'
...
export default async function Home() {
  const [newsRecords, eventRecords, islamicRecords, announcementRecords] = await Promise.all([
    contentRepository.listNews(), contentRepository.listEvents(), contentRepository.listIslamic(), contentRepository.listAnnouncements(),
  ])
  ...
  const popular = [...news].sort((a, b) => (newsRecords.find((item) => item.slug === b.slug)?.viewCount ?? 0) - (newsRecords.find((item) => item.slug === a.slug)?.viewCount ?? 0)).slice(0, 4)
  const upcoming = events.slice(0, 3)
  const islamicHighlights = islamicItems.slice(0, 3)
  const announcements = announcementRecords.slice(0, 3).map((item) => ({ date: formatIndonesianDate(item.publishedAt, false), title: item.title, href: '/pengumuman/' }))
  ...
  return <SiteShell>
    <main id="main-content">
      ...
      <section className="home-editorial container" aria-labelledby="pengumuman-terpopuler"><div className="home-editorial-grid"><div><div className="home-section-heading compact"><div><span className="eyebrow">Informasi</span><h2 id="pengumuman-terpopuler">Pengumuman</h2></div><Link href="/pengumuman/">Lihat semua <ArrowRight size={15} /></Link></div><div className="announcement-list">{announcements.map((item) => <Link key={item.title} href={item.href}><span><small>{item.date}</small><strong>{item.title}</strong></span><ArrowRight size={16} /></Link>)}{announcements.length === 0 && <div className="empty-state compact"><strong>Belum ada pengumuman</strong><p>Belum tersedia informasi resmi.</p></div>}</div></div><div><div className="home-section-heading compact"><div><span className="eyebrow">Pilihan Jamaah</span><h2>Terpopuler</h2></div><Link href="/berita/">Semua berita <ArrowRight size={15} /></Link></div><div className="popular-list">{popular.map((item, index) => <Link key={item.slug} href={`/berita/${item.slug}/`}><span className="popular-rank">0{index + 1}</span><span><small>{item.category}</small><strong>{item.title}</strong></span><ChevronRight size={16} /></Link>)}</div></div></div></section>
      <section className="home-agenda container" aria-labelledby="agenda-terdekat"><div className="home-section-heading"><div><span className="eyebrow">Agenda</span><h2 id="agenda-terdekat">Kegiatan Terdekat</h2></div><Link href="/kegiatan/">Semua kegiatan <ArrowRight size={16} /></Link></div>{upcoming.length > 0 ? <div className="events-grid">{upcoming.map((event) => <Link className="event-card" key={event.slug} href={`/kegiatan/${event.slug}/`}><div className="event-date"><strong>{event.day}</strong><span>{event.month}</span></div><div><span className="event-label">{event.category}</span><h3>{event.title}</h3><p><CalendarDays size={15} /> {event.time}</p><p><MapPin size={15} /> {event.place}</p></div></Link>)}</div> : <div className="empty-state"><strong>Belum ada agenda</strong><p>Belum tersedia kegiatan mendatang.</p></div>}</section>
      <section className="home-islamic container" aria-labelledby="ruang-keislaman"><div className="home-section-heading"><div><span className="eyebrow">Ruang Keislaman</span><h2 id="ruang-keislaman">Kajian &amp; Hikmah</h2></div><Link href="/keislaman/">Semua materi <ArrowRight size={16} /></Link></div>{islamicHighlights.length > 0 ? <div className="islamic-home-grid">{islamicHighlights.map((item, index) => <Link className="islamic-home-card" key={item.slug} href={`/keislaman/${item.slug}/`}><span className="islamic-index">0{index + 1}</span><span className="eyebrow">{item.category}</span><h3>{item.title}</h3><p>{item.excerpt}</p><strong>Pelajari <ArrowRight size={15} /></strong></Link>)}</div> : <div className="empty-state"><strong>Belum ada materi</strong><p>Belum tersedia materi keislaman.</p></div>}</section>
