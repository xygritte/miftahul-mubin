...
const searchItems = [
  'Miftahul Mubin Gelar Kajian Akbar untuk Jamaah dan Warga Sekitar','Menjaga Ukhuwah di Tengah Kesibukan Kehidupan',
  'Program Santunan Miftahul Mubin Kembali Digelar','Pendaftaran Relawan Kegiatan Sosial Masjid Dibuka',
  'Kelas Al-Qur’an untuk Anak dan Remaja Dimulai September','Jadwal Kajian Miftahul Mubin Bulan September',
  'Laporan Keuangan dan Transparansi Bulan Agustus','Jadwal Imam dan Khatib Jumat',
]
...
export default function Header() {
  ...
  return <>
    ...
    {searchOpen&&<div className="search-overlay" role="dialog" aria-modal="true" aria-label="Pencarian"><div className="search-panel"><div className="search-panel-head"><div><span className="eyebrow">Pencarian</span><h2>Cari informasi Miftahul Mubin</h2></div><button className="close-btn" type="button" aria-label="Tutup pencarian" onClick={closeAll}><X size={22}/></button></div><label className="search-field"><Search size={21} aria-hidden="true"/><input autoFocus value={query} onChange={e=>setQuery(e.target.value)} placeholder="Cari berita, kegiatan, atau artikel..."/></label><div className="search-results" aria-live="polite">{!query&&<p className="search-helper">Ketik kata kunci untuk mencari konten pada portal.</p>}{query&&results.length===0&&<p className="search-helper">Tidak ada hasil untuk “{query}”.</p>}{results.map(result=><Link key={result} href="/berita/" onClick={closeAll}><span>{result}</span><span aria-hidden="true">›</span></Link>)}</div></div></div>}
  </>
}
