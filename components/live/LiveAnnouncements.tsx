...
export default function LiveAnnouncements({ initialItems, limit }: Props) {
  ...
  useEffect(() => {
    ...
    async function refresh() {
      if (!supabase) return
      let query = supabase.from('announcements').select('id,title,content,published_at,created_at,updated_at,status').eq('status','published').not('published_at','is',null).lte('published_at',new Date().toISOString()).order('published_at',{ascending:false})
      if (limit) query = query.limit(limit)
      const { data, error } = await query
      if (!error && active) setItems((data ?? []) as AnnouncementRecord[])
    }
    void refresh()
    return () => { active = false }
  }, [limit])
