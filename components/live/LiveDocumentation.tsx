...
function mapMediaItem(row: MediaItemRow): MediaItem | null {
  ...
  if (!url) return null
  return { id: row.id, albumId: row.album_id, type: row.type, title: row.title, url, thumbnailUrl: row.thumbnail_url ?? url, caption: row.caption, sortOrder: row.sort_order, createdAt: row.created_at ?? undefined }
}
...
export default function LiveDocumentation({ initialItems }: { initialItems: MediaItem[] }) {
  ...
  const refresh = useCallback(async () => {
    ...
    if (!error) setItems((data ?? []).map((row) => mapMediaItem(row as MediaItemRow)).filter((item): item is MediaItem => Boolean(item)))
  }, [])

  useEffect(() => { void refresh() }, [refresh])
  useRealtimeRefresh('media_items', refresh)
