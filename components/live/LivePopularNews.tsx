...
type NewsRow = Omit<NewsRecord, 'category'> & { category?: string | { name?: string | null } | null }

function mapNewsRow(row: NewsRow): NewsRecord {
  return { ...row, category: typeof row.category === 'string' ? row.category : row.category?.name ?? 'Berita' }
}
...
export default function LivePopularNews({ initialItems }: { initialItems: NewsRecord[] }) {
  ...
  useEffect(() => {
    ...
    async function refresh() {
      ...
      if (!error && active) setItems(((data ?? []) as unknown as NewsRow[]).map(mapNewsRow))
    }
    void refresh()
    return () => { active = false }
  }, [])
