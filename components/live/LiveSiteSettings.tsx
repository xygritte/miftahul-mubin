...
const empty: Settings = {
  site_name: '', profile_eyebrow: '', profile_title: '', profile_description: '', about_title: '', about_text: '',
...
export default function LiveSiteSettings({ mode }: { mode: 'profile' | 'contact' }) {
  ...
  return <>
    <div className="contact-grid-page">
      <a className="contact-card-page" href={mapLinkUrl} target="_blank" rel="noopener noreferrer"><MapPin size={24}/><span>Alamat</span><h2>{settings.site_name}</h2><p>{settings.contact_address}</p><strong>Buka di Google Maps <ExternalLink size={14}/></strong></a>
...
function PageProfile({ s }: { s: Settings }) {
  ...
  return <>
    <div className="profile-feature"><div className="profile-visual" aria-hidden="true"><span>MM</span><small>{s.site_name}</small></div><div><span className="eyebrow">{s.profile_eyebrow}</span><h2>{s.about_title}</h2><p>{s.about_text}</p>{facts.length > 0 && <div className="profile-facts">{facts.map(([value,label]) => <div key={label}><strong>{value}</strong><span>{label}</span></div>)}</div>}</div></div>
